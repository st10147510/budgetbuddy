...
class StatementParserTest extends TestCase
{
    ...
    // ── Helper: invoke any private method on the job ──────────────────────────
    ...
    {
        $result = $this->invoke('extractTransactions', '', null);
    ...
    public function test_header_lines_are_ignored(): void
    {
        ...
        $result = $this->invoke('extractTransactions', $text, null);
        $this->assertSame([], $result);
    }
    ...
    public function test_full_sa_bank_line_parsed_as_expense(): void
    {
        ...
        $result = $this->invoke('extractTransactions', $line, null);
    ...
    public function test_default_category_used_when_no_keyword_matches(): void
    {
        ...
        $result = $this->invoke('extractTransactions', $line, '3');
    ...
    {
        $result = $this->invoke('parseLine', 'abc', null);
        $this->assertNull($result);
    }

    public function test_zero_amount_returns_null(): void
    {
        $result = $this->invoke('parseLine', '01 May 2026Description-R 0.00R 0.00', null);
        $this->assertNull($result);
    }
    ...
    public function test_date_is_stored_as_milliseconds(): void
    {
        ...
        $result = $this->invoke('parseLine', $line, null);
    ...
    public function test_normalize_english_months_unchanged(): void
    {
        foreach (['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'] as $mon) {
            $result = $this->invoke('normalizeMonth', "01 {$mon} 2025");
    ...
    {
        $this->assertTrue($this->invoke('isIncome', 'Monthly Salary Payment'));
    }

    public function test_deposit_keyword_is_income(): void
    {
        $this->assertTrue($this->invoke('isIncome', 'Cash Deposit'));
    }

    public function test_refund_keyword_is_income(): void
    {
        $this->assertTrue($this->invoke('isIncome', 'Refund from Amazon'));
    }

    public function test_interest_earned_is_income(): void
    {
        $this->assertTrue($this->invoke('isIncome', 'Interest Earned'));
    }

    public function test_transfer_in_is_income(): void
    {
        $this->assertTrue($this->invoke('isIncome', 'Transfer In'));
    }

    public function test_regular_purchase_is_not_income(): void
    {
        $this->assertFalse($this->invoke('isIncome', 'Eskom Payment'));
        $this->assertFalse($this->invoke('isIncome', 'Netflix Subscription'));
        $this->assertFalse($this->invoke('isIncome', 'PnP Supermarket'));
    }
    ...
    {
        $this->assertSame(1, $this->invoke('guessCategory', 'Shoprite Payment'));
    }

    public function test_uber_maps_to_category_2_transport(): void
    {
        $this->assertSame(2, $this->invoke('guessCategory', 'Uber Trip'));
    }

    public function test_netflix_maps_to_category_3_entertainment(): void
    {
        $this->assertSame(3, $this->invoke('guessCategory', 'Netflix Subscription'));
    }

    public function test_dischem_maps_to_category_4_health(): void
    {
        $this->assertSame(4, $this->invoke('guessCategory', 'DisChem Purchase'));
    }

    public function test_eskom_maps_to_category_5_utilities(): void
    {
        $this->assertSame(5, $this->invoke('guessCategory', 'Eskom Prepaid'));
    }

    public function test_rent_maps_to_category_6_housing(): void
    {
        $this->assertSame(6, $this->invoke('guessCategory', 'Monthly Rent Payment'));
    }

    public function test_unisa_maps_to_category_7_education(): void
    {
        $this->assertSame(7, $this->invoke('guessCategory', 'Unisa Tuition Fee'));
    }

    public function test_woolworths_maps_to_category_8_shopping(): void
    {
        $this->assertSame(8, $this->invoke('guessCategory', 'Woolworths Purchase'));
    }

    public function test_savings_maps_to_category_9(): void
    {
        $this->assertSame(9, $this->invoke('guessCategory', 'Transfer to Savings Account'));
    }

    public function test_unknown_merchant_maps_to_category_10(): void
    {
        $this->assertSame(10, $this->invoke('guessCategory', 'Zxqbzz Unknown'));
    }
    ...
    {
        $result = $this->invoke('cleanDesc', '  hello world  ');
